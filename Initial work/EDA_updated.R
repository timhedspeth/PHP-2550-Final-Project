...
figure1 <- ggplot(data = Num_per_year_salmonella, aes(x = year, y = num, color = "Salmonella")) +
  geom_point() + 
  geom_line() +
  scale_x_continuous(breaks=c(seq(2013, 2022, length = 10)),  
                     limits = c(2013, 2022))+
  # E coli 
  ...
  labs(title= "Number of Food-Borne illness cases by year", 
       x = "Year", 
       y = "Number of cases")  + 
  scale_color_discrete(name = "Bacterial Pathogens")

# Print the figure out 
annotate_figure(figure1, 
                bottom = text_grob("Figure 1: Number of cases per year, all illnesses", 
...
Num_per_month_camp <- campylobacter %>%  
  mutate(pathogen="Campylobacter") %>%
  group_by(year, month, pathogen) %>% 
  summarize(num =  n())
Num_per_month_camp <- Num_per_month_camp %>% filter(!is.na(month))


month_plot_dat <- rbind(Num_per_month_sal, Num_per_month_ecoli, Num_per_month_camp)

ggplot(data = month_plot_dat, aes(x = as.numeric(month), y = num)) + 
  ...
  geom_smooth() + 
  theme_minimal() +
  labs(title= "Number of Monthly Cases in Each Year", 
       x = "", 
       y = "Number of cases")  + 
...
ggarrange(plot_by_month_sal, plot_by_month_eco, plot_by_month_cal, nrow =3,
...
# Get the data for the each bacterial pathogens
plotdata_sal <- salmonella %>% mutate(pathogen="Salmonella")%>%
  group_by(year, Isolation.source.category, pathogen) %>% 
  summarize(num = n())

plotdata_ecoli <- ecoli %>% mutate(pathogen="E. coli")%>%
  group_by(year, Isolation.source.category, pathogen) %>% 
  summarize(num = n())

plotdata_camp <- campylobacter %>% mutate(pathogen="Campylobacter")%>%
  group_by(year, Isolation.source.category, pathogen) %>%
  summarize(num = n())

#combine the three data to create plot
plot_dat <- rbind(plotdata_sal, plotdata_ecoli, plotdata_camp)

ggplot(data = plot_dat, aes(x = year, y = num, color = Isolation.source.category)) + 
  facet_grid(rows=vars(pathogen))+
  geom_line() + 
  geom_point() + 
  scale_x_continuous(breaks=c(seq(2013, 2022, length = 10)),  
                     limits = c(2013, 2022))+
  labs(title= "Number of Cases by Isolation Source", 
       x = "", 
       y = "Number of cases",
       color="Isolation Source")+
  theme_minimal() +
  theme(axis.line=element_line(size=0.2),
        legend.position = "bottom")
