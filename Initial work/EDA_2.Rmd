...
## Collection date cleaning 

We note that there is an inherent missingness in the Collection date, but there is no missingness in the Create.date variable, since there is no missingness in Create.Date we will use this as a means of imputing the missing data in the collection date. 
```{r}

# Impute the missing observations for Collection date 
salmonella$Date <- ifelse(is.na(salmonella$Collection.date), salmonella$Create.date, salmonella$Collection.date)

sanity_check_date <- apply(salmonella,2,is.na)
sanity_chck_date_missing <- apply(x,2, sum)

# Now that we have the date imputed we can look at time trends  
salmonella$Date <- as.Date(salmonella$Date)
...
```{r}
# We can change this to the appropriate date
...
regions <- salmonella %>% filter(region != "USA, General")
regions$year <- year(as.Date(regions$Create.date))

cases_by_region <- regions %>% group_by(region, year) %>% 
                               summarize(num = n())

ggplot(data = cases_by_region, aes(x = year, y = num, color = region)) + 
  geom_line() + 
  geom_point()
...
salmonella %>% group_by(Strain) %>% summarize(num = n()) %>% filter(Strain != "" & num >1) 
